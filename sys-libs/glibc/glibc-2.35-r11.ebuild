...
# OpenE2K: changed Python compat to latest version
...
if [[ ${PV} == 9999* ]]; then
	...
	KEYWORDS="~alpha amd64 arm arm64 ~hppa ~loong ~m68k ~mips ppc ppc64 ~riscv ~s390 ~sparc x86 e2k"
	SRC_URI="https://github.com/OpenE2K/${PN}/archive/refs/tags/${PN}-${PV}-${MCST_PV}.tar.gz"
...
# removed gcc dependency, as it's obviously not supports e2k
#		>=${CATEGORY}/gcc-6.2
...
setup_target_flags() {
	...
	case $(tc-arch) in
		...
		e2k)
			# recommended options
			# TODO: maybe validate them? some of them are too sus
			append-flags -fasm-entries-first -fno-strict-aliasing -fno-ipa-attrs -U__NO_INLINE__ -masm-inline \
				-Wno-builtin-functions-redefined -Wno-builtin-declaration-mismatch \
				-Wno-error=invalid-builtin-arg -Wno-error=maybe-uninitialized
		;;
	esac
}
...
src_unpack() {
	...
	if [[ ${PV} == 9999* ]] ; then
		...
		unpack ${P}-${MCST_PV}.tar.gz

		# HACKHACK: normalize directory name
		mv ${PN}-${P}-${MCST_PV} ${P}
...
#	eapply "${FILESDIR}"/2.35/glibc-2.35-make-4.4-MAKEFLAGS.patch
	eapply "${FILESDIR}"/glibc-2.35-e2k-hack.patch
