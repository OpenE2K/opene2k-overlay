...
src_prepare() {
	...
	# fix libutil.so soname typo (LIBUTILMAJOR -> LIBUTILSOMAJOR)
	# the empty -Wl,-soname arg is tolerated by GNU ld but rejected by lcc
	sed -i -e 's/(LIBUTILMAJOR)/(LIBUTILSOMAJOR)/' Makefile || die
