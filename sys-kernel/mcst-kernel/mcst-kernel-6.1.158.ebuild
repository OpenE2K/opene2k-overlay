...
DESCRIPTION="Linux kernel built with MCST patches"
HOMEPAGE="
	https://github.com/OpenE2K/linux
	https://wiki.gentoo.org/wiki/Project:Distribution_Kernel
	https://www.kernel.org/
"
SRC_URI+="
	https://github.com/OpenE2K/linux/archive/refs/tags/v${PV}.tar.gz
		-> ${MY_P}.tar.gz
...
src_prepare() {
	local patch
	[[ -f "${WORKDIR}/patch-${PATCH_PV}" ]] && eapply "${WORKDIR}/patch-${PATCH_PV}"
	[[ -f "${WORKDIR}/${PATCHSET}" ]] && eapply "${WORKDIR}/${PATCHSET}"
	...
	sed -i -e 's:^EXTRAVERSION =[[:space:]]*:EXTRAVERSION = -mcst:' Makefile || die
