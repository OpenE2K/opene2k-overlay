...
if [[ ${PV} == 9999* ]]; then
	...
	SRC_URI="http://setwd.ws/sp/1.29/1.29.16/src/${P}-${MCST_PV}.tar.xz"
	[[ -z ${PATCH_VER} ]] || SRC_URI="${SRC_URI}
		https://setwd.ws/distfiles/${PN}-mcst-${PATCH_BINUTILS_VER}-patches-${PATCH_VER}.tar.xz"
	SLOT=$(ver_cut 1-2)
...
src_unpack() {
	if [[ ${PV} == 9999* ]] ; then
		...
		unpack ${P}-${MCST_PV}.tar.xz

		cd "${WORKDIR}" || die
		unpack ${PN}-mcst-${PATCH_BINUTILS_VER}-patches-${PATCH_VER}.tar.xz
...
src_configure() {
	...
	case ${CTARGET} in
		x86_64-*|aarch64*|arm64*|i[3456]*)
			...
			;;
		e2k*)
			# libopcodes depends on gas and therefore cannot be built shared
			;;
		*)
			myconf+=(
				--enable-shared
